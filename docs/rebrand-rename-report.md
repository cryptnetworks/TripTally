...
| Docker service, container, image, and volume names such as `triptally`, `triptally_data`, and `ghcr.io/cryptnetworks/triptally` | Should remain for compatibility | Preserves current Compose, Cloudflare, Nginx, and GHCR deployment paths.                     |
...
- Local CI image tags and the future GitHub Actions publish target were updated to `seddleup`. The existing pinned GHCR image path remains in operator docs until a new SeddleUp image is published.

## Required Follow-Up

- Decide separately whether to introduce a new GHCR image name such as `ghcr.io/cryptnetworks/seddleup`; if so, publish both old and new image names during a transition period.
